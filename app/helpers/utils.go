...
func ComparePassword(passwordHash, passwordSalt, passwordRequest string) error {
	hash, err := Decrypt(passwordHash)
	if err != nil {
		log.Error().Err(errors.New("ERROR DECRYPT PASSWORD HASH : " + err.Error()))
		return err
	}

	salt, err := Decrypt(passwordSalt)
	if err != nil {
		log.Error().Err(errors.New("ERROR DECRYPT PASSWORD SALT : " + err.Error()))
		return err
	}

	if err := bcrypt.CompareHashAndPassword([]byte(hash), []byte(passwordRequest+salt)); err != nil {
		log.Error().Err(errors.New("INCORRECT PASSWORD")).Msg("")
